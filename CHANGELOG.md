...
- *(tui)* Skip real terminal size query in unit tests

- *(ci)* Revert only direct-dependency bumps in cargo-lock-update

- *(deny)* Resolve cargo-deny findings for memmap2, syn, serde-saphyr

- *(nix)* Replace deprecated nixfmt-rfc-style with nixfmt

- *(deny)* Sync gix/serde-saphyr build bypass versions with Cargo.lock

- *(deny)* Resolve hashbrown/foldhash duplicates, drop stale bypasses
...
- *(tui)* Gate terminal-backend tests behind serial_test
...
- Comment out architecture module

- Remove term quiet config from cargo config

- Add custom Cargo.lock update workflow

- Allow cargo-lock-update PR step to fail gracefully
